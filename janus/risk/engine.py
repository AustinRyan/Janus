...
class RiskEngine:
    ...
    def get_score(self, session_id: str) -> float:
        ...
        if session.last_tool_call_time is not None:
            ...
            if idle_minutes > thresholds.DECAY_IDLE_THRESHOLD_MINUTES:
                decay = thresholds.DECAY_RATE_PER_MINUTE * (idle_minutes - thresholds.DECAY_IDLE_THRESHOLD_MINUTES)
                decayed_score = max(thresholds.MIN_RISK_SCORE, session.risk_score - decay)
    ...
    def update_score(self, session_id: str, delta: float) -> float:
        ...
        new_score = max(thresholds.MIN_RISK_SCORE, min(thresholds.MAX_RISK_SCORE, current + delta))
    ...
    def is_locked(self, session_id: str) -> bool:
        ...
        return self.get_score(session_id) >= thresholds.LOCK_THRESHOLD
    ...
    async def evaluate_risk(
        ...
        recent_events = self.session_store.get_recent_events(
            session_id, window_seconds=thresholds.VELOCITY_WINDOW_SECONDS
        )
        ...
        total_delta = max(thresholds.MIN_RISK_SCORE, min(thresholds.MAX_RISK_SCORE, total_delta))
