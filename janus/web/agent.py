...
class ChatAgent:
    ...
    def set_history(self, history: list[dict[str, Any]]) -> None:
        ...
        # Set original_goal from first user message if not already set
        if not self._original_goal:
            for msg in history:
                if msg.get("role") == "user":
                    self._original_goal = str(msg.get("content", ""))
                    break
                ...
                if block.type == "text":
                    ...
                    if self._exporter_coordinator is not None:
                        try:
                            ...
                            logger.exception("exporter_dispatch_error")

                    # Record tool for predictive risk
                    if hasattr(self._guardian, 'predictor'):
                        self._guardian.predictor.record_tool(
                            self._session_id,
                            block_name,
                        )

                    # Record tool for threat intel
                    if hasattr(
                        self._guardian,
                        'threat_intel_check',
                    ):
                        self._guardian.threat_intel_check.record_tool(
                            self._session_id,
                            block_name,
                        )
